...
python -m registry
    -c
    -i "org.inb.aggregator"
    -n "inbAggregator"
    -u "http://example.org/"
    -d "Testing aggregator aggregator implemented in Python."
...
from .blueprint.services import SERVICES
from .blueprint.query import QUERY
from .blueprint.registered_beacons import REGISTERED_BEACONS
...
def start(debug=True):
    ...
    if not os.path.isfile(AGGREGATOR_CONFIG_FILE):
        abort = True
    ...
    if not abort:
        ...
        API.register_blueprint(SERVICES)
        API.register_blueprint(QUERY)
        API.register_blueprint(REGISTERED_BEACONS)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Aggregator to inform AggregatorAggregators ' + \
    ...
    def flags_parser(parser):
        ...
        parser.add_argument(
            ...
            "Defaults to 5000 for the aggregator."
        )
        parser.add_argument(
            '--debug',
            action='store_true',
    ...
    def service_build(parser):
        ...
        parser.add_argument(
            ...
            '--entry-point',
            default=True,
            help="Use with -c, set if the service is an entry point."
        )
        return parser
...
def build_config_file(args):
    ...
    config.add_section('aggregator')
    config.add_section('organization')
    config.add_section('aggregatorInfo')
    config.add_section('organizationInfo')

    if args.build_config:
        ### ---------------- Aggregator Config build ----------------- ###
        #Handling None types
        try:
            config.set('aggregator', 'id', args.id)
        except TypeError:
            log.error("ID necessary to run the API.")
            abort = True
        try:
            config.set('aggregator', 'welcomeUrl', args.u)
        except TypeError:
            log.warning("WelcomeURL not specified!")
            config.set('aggregator', 'welcomeUrl', "")
        try:
            config.set('aggregator', 'alternativeUrl', args.alternative_url)
        except TypeError:
            config.set('aggregator', 'alternativeUrl', "")

        config.set('aggregator', 'apiVersion', API_VERSION)
        config.set('aggregator', 'version', VERSION)
        config.set('aggregator', 'serviceType', SERVICE_TYPE)
        config.set('aggregator', 'name', args.name)
        config.set('aggregator', 'description', args.description)
        config.set('aggregator', 'open', str(args.open))
        config.set('aggregator', 'entryPoint', "False")
        config.set('aggregator', 'createDateTime', datetime.today().strftime('%Y-%m-%d'))
        config.set('aggregator', 'updateDateTime', EXECUTION_DATE)
        ### -------------------------------------------------------- ###
        ...
        ### -------------------------------------------------------- ###
        if not abort:
            with open(AGGREGATOR_CONFIG_FILE, "w") as file:
                config.write(file)
