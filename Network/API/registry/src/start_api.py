...
python -m registry
    ...
    -n "inbRegistry"
    -u "http://example.org/"
    -d "Testing BeaconNetwork implemented in Python."
...
from .blueprint.networks    import NETWORKS
...
def start():
    ...
    if not os.path.isfile(REGISTRY_CONFIG_FILE):
        abort = True
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Registry to inform BeaconAggregators ' + \
    ...
    def flags_parser(parser):
        ...
        parser.add_argument(
            ...
            "Defaults to 5005 for the registry."
        )
        parser.add_argument(
            '--debug',
            action='store_true',
...
def build_config_file(args):
    ...
    config.add_section('registryInfo')
    ...
    if args.build_config:
        ...
        #Handling None types
        try:
            config.set('registry', 'id', args.id)
        except TypeError:
            log.error("ID necessary to run the API.")
            abort = True
        try:
            config.set('registry', 'welcomeUrl', args.u)
        except TypeError:
            log.warning("WelcomeURL not specified!")
            config.set('registry', 'welcomeUrl', "")
        try:
            config.set('registry', 'alternativeUrl', args.alternative_url)
        except TypeError:
            config.set('registry', 'alternativeUrl', "")

        config.set('registry', 'apiVersion', API_VERSION)
        config.set('registry', 'version', VERSION)
        config.set('registry', 'serviceType', SERVICE_TYPE)
        config.set('registry', 'name', args.name)
        config.set('registry', 'description', args.description)
        config.set('registry', 'open', str(args.open))
        config.set('registry', 'entryPoint', "False")
        config.set('registry', 'createDateTime', datetime.today().strftime('%Y-%m-%d'))
        config.set('registry', 'updateDateTime', EXECUTION_DATE)
        ### -------------------------------------------------------- ###
        ...
        ### -------------------------------------------------------- ###
        if not abort:
            with open(REGISTRY_CONFIG_FILE, "w") as file:
                config.write(file)
